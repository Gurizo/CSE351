...
void* mm_malloc(size_t size) {
  ...
  block_info* ptr_new_free_block = NULL;
  block_info* ptr_new_free_block_footer = NULL;
  size_t block_size;
  ...
  // Search for a free block with requested size.
  // If it does not exists, request more free space and search again.
  ...
  // allocate the block as used.
  //////ptr_free_block->size_and_tags = block_size | preceding_block_use_tag | TAG_USED;
  
  // remove the allocated block from the free block
  remove_free_block(ptr_free_block);
  // if the free block contains enough space for the requested size
  // and another free space greater than the minimum block size, split it.
  if(block_size - req_size >= MIN_BLOCK_SIZE) {
    ptr_free_block->size_and_tags = req_size | preceding_block_use_tag | TAG_USED;

    // create a new free block under given amount of space left in the free block.
    ptr_new_free_block = (block_info*) UNSCALED_POINTER_ADD(ptr_free_block, req_size);
    ptr_new_free_block->size_and_tags = ((block_size - req_size) | TAG_PRECEDING_USED) & ~TAG_USED;
    
    // set a footer for the newly created free block
    ptr_new_free_block_footer = UNSCALED_POINTER_ADD(ptr_free_block, (block_size-WORD_SIZE));
    *((size_t*)ptr_new_free_block_footer) = ((block_size - req_size) | TAG_PRECEDING_USED) & ~TAG_USED; 

    // insert the newly allocated free block.
    insert_free_block(ptr_new_free_block);
  } else { // if there is no enough space to split
    // update the current block as used
    ptr_free_block->size_and_tags = block_size | preceding_block_use_tag | TAG_USED;
    // fetch the next free block.
    block_info* next_free_block = (block_info*) UNSCALED_POINTER_ADD(ptr_free_block, block_size);
    // update the preceding used tag for the next free block
    next_free_block->size_and_tags |= TAG_PRECEDING_USED; 
  }
...
void mm_free(void* ptr) {
  ...
  // fetch the information about the block to free.
  block_to_free = (block_info*)UNSCALED_POINTER_SUB(ptr, WORD_SIZE);
  payload_size = SIZE(block_to_free->size_and_tags);
  // set the header of the block to be freed as unused.
  block_to_free->size_and_tags &= ~TAG_USED;
  // update the footer of the block to be freed.
  *((size_t*) UNSCALED_POINTER_ADD(block_to_free, payload_size-WORD_SIZE)) = block_to_free->size_and_tags;

  
  // update the header of the following_block
  following_block = UNSCALED_POINTER_ADD(block_to_free, payload_size);
  following_block->size_and_tags &= ~TAG_PRECEDING_USED;
  
  // update the footer of the following block if the following block has not been allocated
  if ((following_block->size_and_tags & TAG_USED) == 0) {
    *((size_t*) UNSCALED_POINTER_ADD(following_block, SIZE(following_block->size_and_tags) - WORD_SIZE)) = following_block->size_and_tags;
  }
  // reinsert free block into the head of the list
