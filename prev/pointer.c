...
int int_size() {
  ...
  int size = (char*)int_ptr2 - (char*)int_ptr1;
  return size;
}
...
int double_size() {
  ...
  int size = (char*)doub_ptr2 - (char*)doub_ptr1;
  return size;
}
...
int pointer_size() {
  ...
  // Compute the size of a pointer using pointer arithmetic
  int size = (char*)ptr_ptr2 - (char*)ptr_ptr1;
  return size;
}
...
int within_array(int* int_array, int size, int* ptr) {
  ...
  int byte_offset = (int)((char*)ptr - (char*)start);

  int is_within_array = !(((byte_offset | size) & (byte_offset - size)) >> 31);
